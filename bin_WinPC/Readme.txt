...
　対象OSはWindows XP SP3以降ですが、OlderMachinesバージョンはWindows XPより前のバージョンでも動作します(試せる環境がないためどこまでかは不明)。
...
　なお、このソフトウェアはインストール不要です。削除も、そのまま実行ファイルを削除するだけで大丈夫です。レジストリなどは使用していません。


・Windows以外のOSについて

　基本的なWin32 APIのみを使用しているため、Wine等を使えば起動すると思われます(保証はしません)。Wineについては、Wineの公式サイトを参照してください。
...
　ソースコードは「src」にあります。
　「Visual Studio 2019 Windows XP」がPF_IA-32.exeからPF_AArch64、「Borland C++ Compiler 5.5」がPF_IA-32(OlderMachines).exeのプロジェクトです。
　また、この「bin_WinPC」にバイナリは配置していませんが、GCCでコンパイル可能なソースと、ビルド用batファイルも置いてあります。
...
　このソフトウェアはフリーソフトウェアですが、製作者(watamario)は著作権を放棄しておりませんので、無断転載は禁止です。
　ただし、このプログラムをテンプレートとして作成した全く別のプログラムの配布は認めます。強制はしませんが、ご一報くださると有り難いです。
