...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd) {
    ...
    // default title
    wcscpy_s(wcmes[0], TEXT("素因数分解プログラム - ") TARGET_PLATFORM TEXT(" API ") TARGET_CPU);

    // "about" statement
    wcscpy_s(wcmes[1], TEXT("素因数分解 for Windows PC Ver. 2\n\n")
    ...
    wcscpy_s(wcmes[2], TEXT("入力された自然数を素因数分解します。素数判定にも利用できます。\n")
    ...
    wcscpy_s(wcmes[3], TEXT("入力された範囲で素数の列挙、数え上げを行います。\n")
...
DWORD WINAPI PrimeFactorization(LPVOID arg) {
    ...
    while(1){
        ...
        if(chk) {
            ...
            wcscat_s(wcresult, wcstr); // 結果文字列に追加
        } else {
    ...
    if(cnt==0 && N>1) wcscat_s(wcresult, TEXT(" (素数)")); // 素数だった場合、その旨を追加
    ...
    SendMessage(hedi_out, EM_REPLACESEL, 0, (WPARAM)TEXT("\r\n")); // 続けて書き込むならこの1行だけで良い
    wcscat_s(wcstr, TEXT(" - 素因数分解プログラム"));
