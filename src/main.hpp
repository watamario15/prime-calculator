...
typedef HRESULT(CALLBACK *GetDpiForMonitor_t)(HMONITOR hmonitor, int dpiType, unsigned *dpiX, unsigned *dpiY);

// The function pointer type for MonitorFromWindow API.
typedef HMONITOR(CALLBACK *MonitorFromWindow_t)(HWND hWnd, DWORD dwFlags);

// The function pointer type for SetDllDirectoryW API.
