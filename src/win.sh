...
if [ -n "${PREFIX32-}" ]; then
  ...
  echo "*** [IA-32] Linking..."
  "${PREFIX32}g++" ./*.o -static -s -lcomctl32 -mwindows -municode -o prime-win-32.exe
fi
...
if [ -n "${PREFIX64-}" ]; then
  ...
  echo "*** [AMD64] Linking..."
  "${PREFIX64}g++" ./*.o -static -s -lcomctl32 -mwindows -municode -o prime-win-64.exe
fi
...
if [ -n "${PREFIXA32-}" ]; then
  ...
  "${PREFIXA32}g++" ./*.o -static -s -lcomctl32 -mwindows -municode -o prime-win-arm32.exe
fi
...
if [ -n "${PREFIXA64-}" ]; then
  ...
  "${PREFIXA64}g++" ./*.o -static -s -lcomctl32 -mwindows -municode -o prime-win-arm64.exe
fi
